...
- Raw unsettled transaction list output emits one selected `getUnsettledTransactionListResponse` page at a time. `--page` chooses the gateway page number, `--limit` chooses the selected page's gateway page size, and a later-page indication is emitted as a warning when another raw page exists.
...
- Reports migration metadata in the JSON envelope and structured warnings. `data.result` is a stable status token, `data.message` is the human-readable detail, `data.active_config` identifies the current active config file when present, and `data.migrated_path` is populated only when the command wrote `config.yaml`. The `active_config`, `original_path`, `migrated_path`, and `backup_path` fields are always present in automation output and use an empty string when not applicable.
...
- Accept exact-match `--status`, `--amount`, and `--payment` filters for both settled and unsettled lists; multiple filters combine with AND semantics and sparse filters continue paging until up to `--limit` matching normalized transactions are found or no more gateway pages are available.
- Honor transaction list filter settings with precedence: command flags, `AUTHNET_TX_FILTER_STATUS` / `AUTHNET_TX_FILTER_AMOUNT` / `AUTHNET_TX_FILTER_PAYMENT`, `preferences.transaction_list.filter.status` / `amount` / `payment`, then no filter.
